...
def parse_apache_log(contents: str) -> List[RowDTO]:
    ...
    # Updated regex pattern to capture remote_logname and user
    log_pattern = r'(?P<ip>\S+) (?P<remote_logname>\S+) (?P<user>\S+) \[(?P<timestamp>.*?)\] "(?P<method>\S+) (?P<url>\S+) HTTP/\S+" (?P<status_code>\d+) (?P<response_size>(\d+|-)) "(?P<referrer>.*?)" "(?P<user_agent>.*?)"'
    ...
    for line in contents.splitlines():
        ...
        if match:
            ...
            if 'status_code' in log_data and log_data['status_code']:
                ...
                print(log_data['status_code'])
                status_code = int(log_data['status_code'])
            else:
                status_code = 0  # or provide a default value like 0
            ...
            try:
                ...
                row_dto = RowDTO(
                    ...
                    remote_logname=log_data['remote_logname'],  # Added comma here
                    user=log_data['user']
                )
                rows.append(row_dto)
